...
/**
 * Builds a sequence of SnowflakeUIDs for a particular "machine" instance, which is based on a
 * unique combination of host, process, and process thread.
 */
public class SnowflakeUIDBuilder extends AbstractUIDBuilder<SnowflakeUID> {
   ...
   * Constructor
   *
   * @param nodeId unique 8-bit node ID between 0 and 255, inclusively
   * @param processId unique 6-bit process ID between 0 and 63, inclusively
   * @param threadId unique 6-bit thread ID between 0 and 63, inclusively
   */
   ...
   * Constructor
   *
   * @param timestamp the initial seed value for a 52-bit timestamp (max value of 4503599627370495),
   * or -1 to initialize the builder with a System-generated value based on the current milliseconds
   * from the epoch
   *
   * @param machineId unique 20-bit machine ID between 0 and 1048575, inclusively
   *
   * @param sequenceId the initial seed value for a 24-bit, one-up counter (max value of 16777215),
   * or a negative integer to allow the builder to specify the seed value
   */
   ...
   *
   * @param timestamp the initial seed value for a 52-bit timestamp (max value of 4503599627370495),
   *        or -1 to initialize the builder with a System-generated value based on the current
  ...
  public SnowflakeUID newId(final String... extras) {
    ...
    return (snowflake == UNDEFINED_SNOWFLAKE) ? new SnowflakeUID()
        : new SnowflakeUID(snowflake, radix, extras);
  }
   ...
   *
   * @param options command line options being used
  ...
  protected static int newMachineId(final Map<String,Option> options) {
    ...
    Option option = options.get(UIDConstants.HOST_INDEX_OPT);
    if (null != option) {
      try {
        hostId = Integer.parseInt(option.getValue());
      } catch (final Exception e) {
        if (LOGGER.isDebugEnabled()) {
          final String message = "Invalid " + UIDConstants.HOST_INDEX_OPT + ": " + option;
          LOGGER.warn(message, e);
        }
      }
    }

    option = options.get(UIDConstants.PROCESS_INDEX_OPT);
    if ((hostId >= 0) && (null != option)) {
      try {
        processId = Integer.parseInt(option.getValue());
      } catch (final Exception e) {
        if (LOGGER.isDebugEnabled()) {
          final String message = "Invalid " + UIDConstants.PROCESS_INDEX_OPT + ": " + option;
          LOGGER.warn(message, e);
        }
      }
    }

    option = options.get(UIDConstants.THREAD_INDEX_OPT);
    if ((processId >= 0) && (null != option)) {
      try {
        threadId = Integer.parseInt(option.getValue());
      } catch (final Exception e) {
        if (LOGGER.isDebugEnabled()) {
          final String message = "Invalid " + UIDConstants.THREAD_INDEX_OPT + ": " + option;
          LOGGER.warn(message, e);
        }
      }
    }
  ...
  public String toString() {
    return "SnowflakeUIDBuilder [timestamp=" + tid + ", machineId=" + mid + ", sequenceId=" + sid
        + ", radix=" + radix + "]";
  }

  private static BigInteger validateMachineId(int machineId) {
    if ((machineId < 0) || (machineId > SnowflakeUID.MAX_MACHINE_ID)) {
      throw new IllegalArgumentException("Machine ID must be a value between 0 and "
          + SnowflakeUID.MAX_MACHINE_ID + ", inclusively");
    }
  ...
  private static BigInteger validateMachineIds(int nodeId, int processId, int threadId) {
    ...
    if ((nodeId < 0) || (nodeId > SnowflakeUID.MAX_NODE_ID)) {
      throw new IllegalArgumentException("Node ID must be a value between 0 and "
          + SnowflakeUID.MAX_NODE_ID + ", inclusively, but was " + nodeId);
    }

    if ((processId < 0) || (processId > SnowflakeUID.MAX_PROCESS_ID)) {
      throw new IllegalArgumentException("Process ID must be a value between 0 and "
          + SnowflakeUID.MAX_PROCESS_ID + ", inclusively, but was " + processId);
    }

    if ((threadId < 0) || (threadId > SnowflakeUID.MAX_THREAD_ID)) {
      throw new IllegalArgumentException("Thread ID must be a value between 0 and "
          + SnowflakeUID.MAX_THREAD_ID + ", inclusively, but was " + threadId);
    }
  ...
  private long validateTimestamp(long timestamp) {
    ...
    // removed initial check for timestamp <= 0 since we should ignore any timestamp passed in to
    // this method
    ...
    if (timestamp <= this.previousTid) {
      LOGGER.warn("Current tid is less than the previous.  This could cause uid collisions.\n"
          + "Mid: " + mid + ", Timestamp: " + timestamp + ", Previous: " + previousTid
