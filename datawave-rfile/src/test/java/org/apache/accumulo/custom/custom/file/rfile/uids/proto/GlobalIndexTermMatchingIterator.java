 ...
 * under the License.
 */
package org.apache.accumulo.custom.custom.file.rfile.uids.proto;
...
public class GlobalIndexTermMatchingIterator extends GlobalIndexMatchingFilter
  ...
  private GlobalIndexTermMatchingIterator(GlobalIndexTermMatchingIterator other,
  ...
  public void seek(Range range, Collection<ByteSequence> columnFamilies, boolean inclusive)
    ...
    // if we have been reseeked after being torn down, and we are only returning unique terms, then
    // advance to the next unique row/cf
    if (!range.isStartKeyInclusive() && uniqueTermsOnly && getSource().hasTop()) {
      Key start = range.getStartKey();
      // verify this was actually from being torn down in that this key looks like a global index
      // term
   ...
   *
   * @param top current key that we see
  ...
  protected void advance(final Key top) throws IOException {
    ...
    if (getSource().hasTop() && getSource().getTopKey().compareTo(next) < 0) {
      ...
      getSource().seek(new Range(next, true, scanRange.getEndKey(), scanRange.isEndKeyInclusive()),
          scanCFs, scanInclusive);
    } else {
      if (log.isTraceEnabled())
  ...
  public IteratorOptions describeOptions() {
    ...
    io.addNamedOption(REVERSE_INDEX,
        "Boolean denoting whether we are matching against a reverse index");
    io.addNamedOption(UNIQUE_TERMS_IN_FIELD,
        "Advances the term when one is found, ignoring the fact that the term may exist on multiple shards");
    io.setDescription(
        "GlobalIndexTermMatchingIterator uses a set of literals and regexs to match global index keys");
    return io;
  }
  ...
  public boolean validateOptions(Map<String,String> options) {
    ...
    if (options.containsKey(UNIQUE_TERMS_IN_FIELD)) {
      /*
       * note that boolean will ONLY return true if the value in the map is 'true' or 'TRUE'. we
       * don't need the above conditional, but we are being defensive.
       */
      uniqueTermsOnly = Boolean.valueOf(options.get(UNIQUE_TERMS_IN_FIELD));
