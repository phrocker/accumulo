...
public class HashUID extends UID {
   ...
   * @param time The time to use, can be null (@see Event.getTimeForUID()).
   */
  public HashUID(final byte[] data, final Date time) {
    this(data, time, EMPTY_EXTRAS);
  }

  /**
   * Construct a uid using some data to hash and add some extra on the end. Note that only the
   * hashed portion of the UID is used for shard calculations.
   *
   * @param data The data to hash.
   * @param time The time to use, can be null (@see Event.getTimeForUID()).
   * @param extras Extra stuff to append to the end of the UID, can be null.
   */
   ...
   *
   * @param template the hash-based UID to copy
  ...
  public String getBaseUid() {
    ...
    buf.append(UIDConstants.DEFAULT_SEPARATOR);
    buf.append(Integer.toString(h1, RADIX));
    buf.append(UIDConstants.DEFAULT_SEPARATOR);
    buf.append(Integer.toString(h2, RADIX));
    if (time >= 0) {
      buf.append(UIDConstants.TIME_SEPARATOR);
   ...
   * @throws NumberFormatException if the first part (perhaps a bogus optionalPrefix) can not be
  ...
  @Override
  public int hashCode() {
    return new HashCodeBuilder(19, 37).append(this.optionalPrefix).append(this.h1).append(this.h2)
        .append(this.time).append(this.extra).toHashCode();
  }
   ...
   *
   * @param s the string version of the hash
  ...
  public static HashUID parse(String s, int maxExtraParts) {
    String[] parts = StringUtils.split(s, UIDConstants.DEFAULT_SEPARATOR);
    if (parts.length < 3) {
    ...
    int timeIndex = parts[2].indexOf(UIDConstants.TIME_SEPARATOR);
    int time = -1;
    int b = 0;
    if (timeIndex >= 0) {
    ...
    if (parts.length > 3 && maxExtraParts != 0) {
      ...
      for (int i = 4; i < limit; i++) {
        extra.append(UIDConstants.DEFAULT_SEPARATOR).append(parts[i]);
      }
   ...
   *
   * @param s string representation of hash
   * @return UID
   */
  @SuppressWarnings("unchecked")
