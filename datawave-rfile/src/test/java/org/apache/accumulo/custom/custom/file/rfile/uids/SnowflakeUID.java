...
/**
 * Internal, DATAWAVE-specific, unique identifier. Instead of using a UID based on hash values,
 * however, this class uses a 96-bit ID based on a 52-bit timestamp, 20-bit machine ID, and a 24-bit
 * one-up sequence ID.
 * <p>
 * In order to achieve uniqueness, no two threads should use the same machine ID at the same time
 * anywhere in the cluster. Although there are many ways to manage unique machine IDs, each ID is
 * presumed to have been calculated from an 8-bit node ID, a 6-bit process ID, and a 6-bit thread
 * ID. In other words, each machine ID uniquely identifies a single thread within a particular JVM
 * on a particular host.
 * <p>
 * A {@link SnowflakeUID} cannot be used like a {@link HashUID} to recreate equivalent instances
 * from duplicate data. The trade-off is that it helps contiguous entries compress <i>much</i>
 * better in Accumulo.
 */
public class SnowflakeUID extends UID {
  ...
  private static final BigInteger TIMESTAMP_MASK =
      BigInteger.valueOf(4503599627370495L).shiftLeft(44);
   ...
   *
   * @param rawId raw 96-bit numerical value
   * @param radix used for creating a String representation of this ID, such as 10 for decimal and
   *        16 for hexadecimal (the default)
   * @param extras additional "stuff", if any, to append to the end of the UID, which can be null
   */
   ...
   *
   * @param template the Snowflake-based UID to copy
   * @param extras extra values, if any, to tack onto the UID
   */
  ...
  /**
   * Returns a builder for creating uninitialized Snowflake UIDs used ONLY for deserialization
   * purposes
   *
   * @return a builder for creating uninitialized Snowflake UIDs used ONLY for deserialization
   *         purposes
   */
  ...
  /**
   * Creates a Snowflake-based UID builder based on the node, process, and thread responsible for
   * queuing the generation of such UIDs
   *
   * @param nodeId one-up, 8-bit index value for the staging host responsible for queuing the
   *        generation of this UID
   * @param processId one-up, 6-bit index value for the process responsible for queuing the
   *        generation of this UID
   * @param threadId one-up, 6-bit index value for the process thread responsible for queuing the
   *        generation of this UID
  ...
  /**
   * Creates a Snowflake-based UID builder based on an initial timestamp and the node, process, and
   * thread responsible for queuing the generation of such UIDs
   *
   * @param timestamp the initial timestamp to use for generating UIDs
   * @param nodeId one-up, 8-bit index value for the staging host responsible for queuing the
   *        generation of this UID
   * @param processId one-up, 6-bit index value for the process responsible for queuing the
   *        generation of this UID
   ...
   * @return a builder used for creating Snowflake-based UIDs
   */
  protected static SnowflakeUIDBuilder builder(long timestamp, int nodeId, int processId,
      int threadId) {
    return new SnowflakeUIDBuilder(timestamp, nodeId, processId, threadId);
  }

  /**
   * Creates a Snowflake-based UID builder based on an initial timestamp, sequence ID, and the node,
   * process, and thread responsible for queuing the generation of such UIDs
   *
   * @param timestamp the initial timestamp to use for generating UIDs
   * @param nodeId one-up, 8-bit index value for the staging host responsible for queuing the
   *        generation of this UID
   ...
   * @param threadId one-up, 6-bit index value for the process thread responsible for queuing the
   *        generation of this UID
   * @param sequenceId the initial 24-bit sequence value to use for generating UIDs
   * @return a builder used for creating Snowflake-based UIDs
   */
  protected static SnowflakeUIDBuilder builder(long timestamp, int nodeId, int processId,
      int threadId, int sequenceId) {
    return new SnowflakeUIDBuilder(timestamp, nodeId, processId, threadId, sequenceId);
  }

  /**
   * Creates a {@link SnowflakeUID} builder based on the specified machine ID, which must be a
   * non-negative integer no greater than 20 bits in length (i.e., 0 to 1048575).
   *
   * @param machineId a non-negative integer no greater than 20 bits in length
  ...
  /**
   * Creates a Snowflake-based UID builder based on an initial timestamp and a 20-bit unique ID of
   * the machine responsible for queuing the generation of such UIDs
   *
   * @param timestamp the initial timestamp to use for generating UIDs
   * @param machineId unique ID of the machine responsible for queuing the generation of such UIDs
   * @return a builder used for creating Snowflake-based UIDs
   */
  ...
  /**
   * Creates a Snowflake-based UID builder based on an initial timestamp, sequence ID, and a 20-bit
  ...
  @Override
  public int getTime() {
    return (int) (getTimestamp() % UIDConstants.MILLISECONDS_PER_DAY);
  }
  ...
  /**
   * Returns the underlying BigInteger value of this UID, which may be null if constructed with the
   * no-arg constructor
   *
  ...
  @Override
  public int hashCode() {
    return new HashCodeBuilder(19, 37).append(this.optionalPrefix).append(this.snowflake)
        .append(this.extra).toHashCode();
  }

  /**
   * Parses the string representation of the UID using the default radix and an unspecified number
   * of extra parts
   *
   * @param s string of the UID
   * @return SnowflakeUID
   */
  ...
  /**
   * Parses the string representation of the UID using the default radix, but only include up to
   * maxExtraParts of the portion past the base hash
   *
   * @param s string of the UID
   * @param maxExtraParts is the number of pieces of the extra portion to include. -1 means all, 0
   *        means none.
   * @return UID
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Parses the string representation of the UID using the specified radix, but only include up to
   * maxExtraParts of the portion past the base hash
   *
   * @param s string of the UID
  ...
  public static SnowflakeUID parse(final String s, int radix, int maxExtraParts) {
    final String[] parts =
        (null != s) ? StringUtils.split(s, UIDConstants.DEFAULT_SEPARATOR) : EMPTY_STRINGS;
    if (parts.length < 1) {
    ...
    if ((parts.length > 1) && (maxExtraParts != 0)) {
      ...
      for (int i = 1; i < limit; i++) {
        if (i > 1) {
          extra.append(UIDConstants.DEFAULT_SEPARATOR);
        }
  ...
  /**
   * Generate the string representation of the Snowflake-based UID using a different radix from the
   * default
   *
   * @param radix the desired radix
  ...
  public String toString(int radix) {
    ...
    if ((null != extra) && !extra.isEmpty()) {
      builder.append(UIDConstants.DEFAULT_SEPARATOR);
