...
import static org.apache.accumulo.custom.custom.file.rfile.uids.UIDConstants.CONFIG_MACHINE_ID_KEY;
import static org.apache.accumulo.custom.custom.file.rfile.uids.UIDConstants.CONFIG_UID_TYPE_KEY;
import static org.apache.accumulo.custom.custom.file.rfile.uids.UIDConstants.DEFAULT_SEPARATOR;
import static org.apache.accumulo.custom.custom.file.rfile.uids.UIDConstants.MILLISECONDS_PER_DAY;
...
/**
 * Internal, DATAWAVE-specific, unique identifier. Instead of using a UUID which consumes 128 bits,
 * we are using:
 *
 * originally: two concatenated int values that are results of computing a Murmur hash on the raw
 * bytes using two different seeds. The resulting UID has the form:
 *
 * OptionPrefix.Hash1.Hash2
 *
 * currently: three concatenated int values that are results of computing a Murmur hash on the raw
 * bytes using three different seeds. The resulting UID has the form:
 *
 * Hash0.Hash1.Hash2
 *
...
public abstract class UID implements Comparable<UID>, Comparator<UID>, Writable, Serializable {
   ...
   * Constructor
   *
   * @param prefix Extra stuff to prepend to the UID, which can be null if specified as optional
   * @param isPrefixOptional if true, the prefix is optional and should not be validated for null
   * @param extras Extra stuff to append to the UID, which can be null
   */
  protected UID(final String prefix, boolean isPrefixOptional, final String... extras) {
    if (!isPrefixOptional && (null == prefix)) {
      throw new IllegalArgumentException(
          "Prefix is not optional and must be non-null to use this constructor");
    }
  ...
  /**
   * Extract the number of milliseconds from the beginning of the day. Note that this is not
   * entirely accurate but is significantly faster than using GregorianCalendar operations.
   *
   * @param time date object to extract from
  ...
  /**
   * Get the portion of the UID to be used for sharding (@see
   * datawave.ingest.mapreduce.handler.shard.ShardIdFactory)
   *
   ...
   *
   * @param config Hadoop configuration, or null if a default builder is satisfactory
   ...
   *
   * @param time the time value for the builder
   * @return a default UID builder
   */
   ...
   *
   * @param config current configuration settings
   * @param time a Date-based timestamp
  ...
  public static UIDBuilder<UID> builder(final Configuration config, final Date time) {
    ...
    if (type == null || HashUID.class.getSimpleName().equals(type)) {
      ...
      if (machineId >= 0) {
        ...
        LOGGER.warn(
            "Attempting to generate snowflake ids without caching could cause uid collisions in the event of clock roll-back");

        builder = (UIDBuilder) SnowflakeUID.builder(machineId);
      } else {
        final String message =
            "A 20-bit, non-negative, integer Machine ID must be configured with the "
                + CONFIG_MACHINE_ID_KEY + " property key in order to build "
                + SnowflakeUID.class.getSimpleName() + "s";
      ...
      try {
        ...
        throw new IllegalArgumentException("Unable to find appropriate builder method for " + type,
            e);
   ...
   *
   * @param <UIDTYPET> expected class of the UID.
   * @param s string version of hash
   * @return UID of parsed hash
   */
  @SuppressWarnings("unchecked")
  public static <UIDTYPET extends UID> UIDTYPET parse(final String s) {
    return parse(s, -1);
  }

  /**
   * Parses the string representation of the hash, but only include up to maxExtraParts of the
   * portion past the base hash
   *
   * @param <UIDTYPET> expected class of the UID.
   * @param s string version of hash
   ...
   */
  @SuppressWarnings("unchecked")
  public static <UIDTYPET extends UID> UIDTYPET parse(final String s, int maxExtraParts) {
    if ((null == s) || s.isEmpty()) {
    ...
    final UIDTYPET uid;
    if (parts.length < 3) {
      uid = (UIDTYPET) SnowflakeUID.parse(s, maxExtraParts);
    } else {
      if (parts[0].length() > 8) {
        uid = (UIDTYPET) SnowflakeUID.parse(s, maxExtraParts);
      } else {
        uid = (UIDTYPET) HashUID.parse(s, maxExtraParts);
      }
    }
   ...
   * @return UID of parsed hash
   */
  public static <UIDTYPET extends UID> UIDTYPET parseBase(String s) {
    return parse(s, 0);
  }
